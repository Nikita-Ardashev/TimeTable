...
const BoxDropdown = styled.FlatList`
  display: flex;
  flex-direction: column;
  background-color: white;
  width: 100%;
  max-height: ${Dimension - 64 - 24}px;
  padding-bottom: 24px;
  position: absolute;
  top: 64px;
  z-index: 0;
...
export const Dropdown = ({ ArrayForDropdown, isDisplayBox, setDisplayBox }) => {
      ...
      return (
        <Wrapper>
          ...
          <BoxDropdown
            ...
            renderItem={({ item }) => (
              <ItemDropdown>
                <ItemTextDropdown title={item.value}>
                  {item.value}
                </ItemTextDropdown>
              </ItemDropdown>
            )}
